...
class DiceService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      // Пытаемся прочитать стиль кубиков из предыдущих бросков (если были)
      ...
      // Всегда считаем 3D доступным — если Dice Extension не установлен,
      // первый бросок просто вернёт таймаут и мы переключимся на локальный
  ...
  async roll(formula: string, label?: string, unitName?: string): Promise<DiceRollResult> {
    ...
    try {
      const result = await this.roll3D(formula, label, unitName);
      // Успешный бросок — сбрасываем счётчик неудач
      this.failedAttempts = 0;
      return result;
    } catch (error) {
      console.warn('[DiceService] 3D бросок не удался, переключаюсь на локальный режим:', error);
      
      // Увеличиваем счётчик неудач
      this.failedAttempts++;
      
      // Если слишком много неудач — отключаем 3D чтобы не ждать таймаут каждый раз
      if (this.failedAttempts >= this.MAX_FAILED_ATTEMPTS) {
        console.warn(`[DiceService] ${this.failedAttempts} неудачных попыток, отключаю 3D кубики`);
        this.diceAvailable = false;
      }
  ...
  /**
   * Сбросить состояние и попробовать снова 3D кубики
   * (можно вызвать из настроек)
   */
  resetAndRetry3D(): void {
    this.diceAvailable = true;
    this.failedAttempts = 0;
    console.log('[DiceService] Сброс состояния, пробуем 3D кубики снова');
  }
