...
function checkStepCondition(condition: any, context: CastContext): boolean {
    ...
    const val = condition.key ? context.values[condition.key] : undefined;
    
    switch (condition.type) {
      case 'value_equals': return val != undefined && String(val) === String(condition.value);
      case 'value_gte': return typeof val === 'number' && val >= Number(condition.value);
      case 'value_lte': return typeof val === 'number' && val <= Number(condition.value);
