...
export function CardsTab() {
  ...
  const handleRollCards = async (count: number) => {
    ...
    try {
      // Тратим карты
      spendResource(unit.id, rokDeck.id, count);
      ...
      while (cardsToProcess > 0 || extraCardsToRoll > 0) {
        ...
        // Бросок на попадание
        const hitRoll = rollDice('d20', `Карта ${cardIndex} попадание`);
        const isHit = hitRoll.total >= 11;
        
        // Бросок на эффект
        const effectRoll = rollDice('d20', `Карта ${cardIndex} эффект`);
        const effect = getRokEffect(effectRoll.total);
        ...
        if (effect.additionalRolls) {
          for (const roll of effect.additionalRolls) {
            const result = rollDice(roll.dice, roll.label);
        ...
        if (effect.requiresSuccessCheck) {
          const successCheck = rollDice('d20', 'Проверка успеха');
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={result.isHit ? 'text-green-500' : 'text-blood-bright'}>
