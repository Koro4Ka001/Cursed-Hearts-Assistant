...
export function MagicTab() {
  ...
  const handleCast = async () => {
    ...
    const castRoll = await diceService.roll(`d20+${magicBonus}`, `Каст: ${selectedSpell.name}`);
    ...
    if (selectedSpell.type === 'self' || selectedSpell.type === 'summon') {
      ...
      for (let i = 0; i < selectedSpell.projectiles; i++) {
        if (selectedSpell.canDodge) {
          const hitRoll = await diceService.roll(`d20+${magicBonus}`, `Снаряд ${i + 1}`);
          ...
          if (hit && selectedSpell.damageFormula) {
            ...
            const dmgRoll = await diceService.roll(`${dmgF}+${intBonus}`, `Урон снаряда ${i + 1}`);
            results.push({
              label: `  → Урон`,
              roll: dmgRoll.total - intBonus, bonus: intBonus, total: dmgRoll.total,
          ...
          if (selectedSpell.damageFormula) {
            const dmgRoll = await diceService.roll(`${selectedSpell.damageFormula}+${intBonus}`, `Авто-снаряд ${i + 1}`);
            results.push({
              label: `Снаряд ${i + 1}`,
              roll: dmgRoll.total - intBonus, bonus: intBonus, total: dmgRoll.total,
              success: true,
  ...
  return (
    <div className="space-y-2 animate-[fadeSlideIn_200ms]">
      <ProgressBar current={unit.mana.current} max={unit.mana.max} type="mana" icon="💠"
