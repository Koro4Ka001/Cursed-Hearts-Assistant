...
/**
 * Проверяет, содержит ли projectiles формулу с кубиком
 */
...
export function MagicTab() {
  ...
  const handleCast = async () => {
    ...
    try {
      ...
      // 2. Бросок на каст (для обычных заклинаний — перед оплатой, чтобы учесть крит)
      if (!selectedSpell.isMultiStep) {
        ...
        // Крит 20 = стоимость маны вдвое дешевле
        ...
        if (!castSuccess) {
          ...
          if (costType === 'mana') {
            const failCost = baseCost;
            if (currentMana >= failCost) {
              await spendMana(unit.id, failCost);
              log.push(`💠 Потрачено ${failCost} маны`);
            }
          } else {
        ...
        log.push(`✅ Каст успешен! [${castResult.rawD20 ?? '?'}] + ${magicBonus} = ${castResult.total}${castCritDiscount ? ' — ✨КРИТ! Мана ×0.5!' : ''}`);
      }
      
      // 3. Списываем ману/HP (с учётом крит-скидки)
      ...
      if (costType === 'mana') {
        ...
        log.push(`💠 Потрачено ${finalCost} маны${castCritDiscount ? ` (было ${baseCost}, крит ×0.5)` : ''}`);
      } else {
        ...
        switch (spellType) {
          ...
          case 'aoe':
            if (selectedSpell.damageFormula && selectedSpell.damageType) {
              ...
              log.push(`💥 АОЕ урон: [${aoeResult.rolls.join(', ')}] + ${totalBonus} = ${aoeResult.total} ${damageTypeName}`);
          ...
          case 'targeted': {
            ...
            if (selectedSpell.isMultiStep) {
              ...
              log.push(`✅ Шаг 1 — Попадание: [${hitRoll}]${hitRoll === 20 ? ' — ✨КРИТ! Чистый урон + ×2 кубики!' : ' — Попадание!'}`);
              ...
              // Шаг 4: Бросок урона по формуле tier'а (×2 кубики при крите)
              const isCrit = hitRoll === 20;
              const dmgFormula = totalBonus > 0
                ? `${tier.formula}+${totalBonus}`
                : tier.formula;
              
              // При крите rollWithCrit удваивает количество кубиков
              const dmgResult = await diceService.rollWithCrit(dmgFormula, isCrit, `Урон (${tierLabel})`);
              ...
              const critDmgText = isCrit ? ' (×2 кубики!)' : '';
              log.push(`💥 Шаг 4 — Урон${critDmgText}: [${dmgResult.rolls.join(', ')}]${totalBonus > 0 ? ` + ${totalBonus}` : ''} = ${dmgResult.total} ${damageTypeName}`);
                ...
                {selectedSpell.projectiles && (
                  <>
                    ...
                    <span className="text-ancient">
                      {projectileHasFormula ? `${selectedSpell.projectiles} снарядов` : `${selectedSpell.projectiles} снаряд(ов)`}
                    </span>
                  </>
                )}
              </div>
