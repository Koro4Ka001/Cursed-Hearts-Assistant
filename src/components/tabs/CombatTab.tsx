...
import type { DiceRollResult, DamageType, DamageCategory, Weapon, Resource } from '../../types';
...
export function CombatTab() {
  ...
  const [rangedAttackResults, setRangedAttackResults] = useState<DiceRollResult[]>([]);
  ...
  const meleeWeapons = unit.weapons.filter(w => w.type === 'melee');
  const rangedWeapons = unit.weapons.filter(w => w.type === 'ranged');
  
  // Боеприпасы (ресурсы с типом ammo)
  const ammoResources = unit.resources.filter(r => r.resourceType === 'ammo');
  ...
  const handleMeleeAttack = async () => {
    ...
    try {
      for (let target = 0; target < meleeTargetCount; target++) {
        // Бросок на попадание
        const profBonus = unit.proficiencies[selectedMeleeWeapon.proficiencyType];
        const hitBonus = profBonus + selectedMeleeWeapon.hitBonus;
        ...
        const baseDamageFormula = selectedMeleeWeapon.damageFormula;
        ...
        await diceService.announceDamage(
          unit.shortName,
          damageResult.total,
          DAMAGE_TYPE_NAMES[selectedMeleeWeapon.damageType],
          damageResult.rolls,
          statBonus,
          isCrit
        );
        ...
        if (selectedMeleeWeapon.extraDamageFormula && selectedMeleeWeapon.extraDamageType) {
          const extraResult = rollWithCrit(
            ...
            isCrit,
            `Доп. урон (${DAMAGE_TYPE_NAMES[selectedMeleeWeapon.extraDamageType]})`
          );
          ...
          await diceService.announceDamage(
            unit.shortName,
            extraResult.total,
            DAMAGE_TYPE_NAMES[selectedMeleeWeapon.extraDamageType],
            extraResult.rolls,
            0,
            isCrit
          );
  ...
  const handleRangedAttack = async () => {
    ...
    if (selectedAmmo.current < totalArrowsNeeded) {
      await diceService.showNotification(`❌ Недостаточно ${selectedAmmo.name}! Нужно ${totalArrowsNeeded}, есть ${selectedAmmo.current}`);
      return;
    }
    
    setIsRangedAttacking(true);
    setRangedAttackResults([]);
    ...
    try {
      ...
      for (let shot = 0; shot < rangedShotCount; shot++) {
        ...
        for (let arrow = 0; arrow < arrowsPerShot; arrow++) {
          ...
          // Бросок на попадание
          const profBonus = unit.proficiencies[selectedRangedWeapon.proficiencyType];
          const hitBonus = profBonus + selectedRangedWeapon.hitBonus;
          const hitFormula = hitBonus >= 0 ? `d20+${hitBonus}` : `d20${hitBonus}`;
          ...
          if (isCritFail) {
            ...
            await diceService.announceMiss(unit.shortName, selectedAmmo.name, hitResult);
            continue;
          }
          ...
          if (selectedAmmo.damageFormula && selectedAmmo.damageType) {
            ...
            const damageFormula = dexBonus > 0
              ? `${selectedAmmo.damageFormula}+${dexBonus}`
              : selectedAmmo.damageFormula;
            
            const damageResult = rollWithCrit(damageFormula, isCrit, `Урон ${selectedAmmo.name}`);
            newDamageResults.push(damageResult);
            
            const critText = isCrit ? '✨ КРИТ! ' : '';
            log.push(`🎯 Стрела ${arrow + 1}: [${hitResult.rawD20}] + ${hitBonus} = ${hitResult.total} ${critText}→ 💥 ${damageResult.total} ${DAMAGE_TYPE_NAMES[selectedAmmo.damageType]}`);
            
            await diceService.announceDamage(
              unit.shortName,
              damageResult.total,
              DAMAGE_TYPE_NAMES[selectedAmmo.damageType],
              damageResult.rolls,
              dexBonus,
              isCrit
            );
            ...
            if (selectedAmmo.extraDamageFormula && selectedAmmo.extraDamageType) {
              const extraResult = rollWithCrit(
                ...
                isCrit,
                `Доп. урон (${DAMAGE_TYPE_NAMES[selectedAmmo.extraDamageType]})`
              );
              newDamageResults.push(extraResult);
              
              log.push(`    + ${extraResult.total} ${DAMAGE_TYPE_NAMES[selectedAmmo.extraDamageType]}`);
              
              await diceService.announceDamage(
                unit.shortName,
                extraResult.total,
                DAMAGE_TYPE_NAMES[selectedAmmo.extraDamageType],
                extraResult.rolls,
                0,
                isCrit
              );
      ...
      setResource(unit.id, selectedAmmo.id, selectedAmmo.current - arrowsUsed);
      log.push(`📦 Списано ${arrowsUsed} ${selectedAmmo.name}`);
      
    } finally {
      setRangedAttackResults(newAttackResults);
          ...
          <div className="space-y-3">
            ...
            {selectedMeleeWeapon && (
              <div className="text-xs text-faded">
                {selectedMeleeWeapon.damageFormula} {DAMAGE_TYPE_NAMES[selectedMeleeWeapon.damageType]} | 
                Владение +{unit.proficiencies[selectedMeleeWeapon.proficiencyType]}
                {selectedMeleeWeapon.hitBonus > 0 && ` | Бонус +${selectedMeleeWeapon.hitBonus}`}
          ...
          <div className="space-y-3">
            <Select
              ...
              options={rangedWeapons.map(w => ({ 
                value: w.id, 
                label: `${w.name}${w.multishot > 1 ? ` (×${w.multishot})` : ''}` 
              }))}
            />
            ...
            <Select
              ...
              options={ammoResources.map(r => ({ 
                value: r.id, 
                label: `${r.icon} ${r.name} (${r.current}/${r.max}) — ${r.damageFormula ?? 'нет урона'}` 
              }))}
            />
            
            {selectedRangedWeapon && selectedAmmo && (
              <div className="text-xs text-faded p-2 bg-obsidian rounded border border-edge-bone">
                <div>🏹 {selectedRangedWeapon.name}: +{selectedRangedWeapon.hitBonus + unit.proficiencies.bows} к попаданию</div>
                {selectedRangedWeapon.multishot > 1 && (
                  <div className="text-ancient">⚡ {selectedRangedWeapon.multishot} стрел за выстрел</div>
                )}
                <div className="mt-1">
                  🎯 {selectedAmmo.name}: {selectedAmmo.damageFormula} {selectedAmmo.damageType && DAMAGE_TYPE_NAMES[selectedAmmo.damageType]}
                </div>
                {selectedAmmo.extraDamageFormula && (
                  <div className="text-mana-bright">+ {selectedAmmo.extraDamageFormula} {selectedAmmo.extraDamageType && DAMAGE_TYPE_NAMES[selectedAmmo.extraDamageType]}</div>
                )}
              </div>
            )}
            ...
            <Button
              ...
              disabled={!selectedRangedWeapon || !selectedAmmo || selectedAmmo.current < (selectedRangedWeapon?.multishot ?? 1)}
              className="w-full"
            >
              🏹 ВЫСТРЕЛИТЬ
            </Button>
