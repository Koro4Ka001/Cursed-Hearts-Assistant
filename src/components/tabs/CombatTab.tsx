...
export function CombatTab() {
  ...
  const handleMeleeAttack = async () => {
    ...
    for (let i = 0; i < targetCount; i++) {
      // Hit roll via diceService (broadcasts to all)
      const hitRoll = await diceService.roll(`d20+${profBonus}`, `${unit.shortName} атакует (${weapon.name})`);
      const rawD20 = hitRoll.rolls[0] || 0;
      const hit = hitRoll.isCrit || (!hitRoll.isCritFail && hitRoll.total > 11);

      results.push({
        label: `Цель ${i + 1}`,
        roll: rawD20, bonus: profBonus, total: hitRoll.total,
      ...
      if (hit && weapon.damageFormula) {
        ...
        // Crit: double dice count
        const dmgFormula = hitRoll.isCrit
          ? doubleDiceFormula(weapon.damageFormula)
          : weapon.damageFormula;
        const dmgRoll = await diceService.roll(
          `${dmgFormula}+${statBonus}`,
          `Урон (${weapon.name})${hitRoll.isCrit ? ' КРИТ×2🎲' : ''}`
        );
        results.push({
          label: `  → Урон`,
          roll: dmgRoll.total - statBonus, bonus: statBonus, total: dmgRoll.total,
  ...
  const handleRangedAttack = async () => {
    ...
    for (let i = 0; i < arrowCount; i++) {
      const hitRoll = await diceService.roll(`d20+${hitBonus}`, `${unit.shortName} стреляет (${bow.name})`);
      const rawD20 = hitRoll.rolls[0] || 0;
      const hit = hitRoll.isCrit || (!hitRoll.isCritFail && hitRoll.total > 11);

      results.push({
        label: `Стрела ${i + 1}`,
        roll: rawD20, bonus: hitBonus, total: hitRoll.total,
        success: hit, isCrit: hitRoll.isCrit, isCritFail: hitRoll.isCritFail,
      });

      if (hit && arrow.damageFormula) {
        const statBonus = getStatDamageBonus('dexterity', unit);
        const dmgFormula = hitRoll.isCrit ? doubleDiceFormula(arrow.damageFormula) : arrow.damageFormula;
        const dmgRoll = await diceService.roll(
          `${dmgFormula}+${statBonus}`,
          `Урон стрелой (${arrow.name})`
        );
        results.push({
          label: `  → Урон`,
          roll: dmgRoll.total - statBonus, bonus: statBonus, total: dmgRoll.total,
      ...
      <Section title="Ближний бой" icon="⚔️" collapsible>
        {meleeWeapons.length === 0 ? (
          ...
          <div className="space-y-2">
            <Select label="Оружие" value={selectedWeaponId} onChange={(e) => setSelectedWeaponId(e.target.value)}
            ...
            <Button variant="primary" className="w-full" onClick={handleMeleeAttack} disabled={!selectedWeaponId || isRolling} loading={isRolling}>
              ⚔️ Атаковать
            </Button>
      ...
      <Section title="Дальний бой" icon="🏹" collapsible defaultOpen={false}>
        {rangedWeapons.length === 0 ? (
          ...
          <div className="space-y-2">
            ...
            <Button variant="primary" className="w-full" onClick={handleRangedAttack} disabled={!selectedBowId || !selectedArrowId || isRolling} loading={isRolling}>
              🏹 Выстрелить
            </Button>
...
// Double dice count for crits: "5d20" -> "10d20"
function doubleDiceFormula(formula: string): string {
