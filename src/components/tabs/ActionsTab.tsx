...
export function ActionsTab() {
  ...
  const executeAction = (action: QuickAction) => {
    ...
    const processStep = (step: ActionStep, stepNum: number) => {
      ...
      results.push({
        label: `Шаг ${stepNum}`,
        roll: roll.rolls[0], bonus, total: roll.total,
            ...
            {unit.quickActions.map((action) => (
              <div key={action.id} className="flex items-center gap-1">
                <button
                  ...
                  className="flex-1 flex items-center gap-2 px-2.5 py-2 bg-[#161412] rounded-lg border border-[#3a332a] hover:border-[#7a6f62] transition-all text-left cursor-pointer"
                >
    ...
    <Modal isOpen={true} onClose={onClose} title={action ? 'Редактировать' : 'Создать действие'} maxWidth="max-w-lg">
      <div className="space-y-2">
        ...
        <div className="flex gap-2 pt-1">
          <Button variant="gold" className="flex-1" onClick={handleSave} disabled={!name.trim()}>Сохранить</Button>
...
function StepEditor({ step, index, allSteps, onUpdate, onRemove }: {
  ...
  const nextStepOptions = allSteps
    ...
    .map((s, i) => ({ value: s.id, label: `Шаг ${i + 1}` }));
      ...
      <div className="grid grid-cols-2 gap-1.5">
        <div>
          ...
          {step.onSuccess.type === 'next_step' && nextStepOptions.length > 0 && (
            <Select value={step.onSuccess.nextStepId || ''}
              onChange={(e) => onUpdate({ onSuccess: { ...step.onSuccess, nextStepId: e.target.value } })}
              options={[{ value: '', label: '—' }, ...nextStepOptions]} />
          )}
          <input
            className="w-full h-7 px-2 mt-0.5 bg-[#0c0a09] text-[#b8a892] text-[10px] rounded border border-[#3a332a] focus:outline-none focus:border-[#d4a726]"
        ...
        <div>
          ...
          {step.onFailure.type === 'next_step' && nextStepOptions.length > 0 && (
            <Select value={step.onFailure.nextStepId || ''}
              onChange={(e) => onUpdate({ onFailure: { ...step.onFailure, nextStepId: e.target.value } })}
              options={[{ value: '', label: '—' }, ...nextStepOptions]} />
          )}
          <input
            className="w-full h-7 px-2 mt-0.5 bg-[#0c0a09] text-[#b8a892] text-[10px] rounded border border-[#3a332a] focus:outline-none focus:border-[#d4a726]"
